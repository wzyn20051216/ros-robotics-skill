...
def detect_package(package_xml: Path) -> PackageInfo:
    """! @brief 根据 `package.xml` 与同目录构建文件判断包类型。"""
...
def iter_package_xml_files(root: Path) -> Iterable[Path]:
    """! @brief 遍历工作区中的 `package.xml`，跳过常见构建产物目录。"""
...
def detect_workspace(root: Path) -> DetectionResult:
    ...
    if workspace_type == "ros1-catkin":
        ...
        recommended_commands = [
            ...
            "分别 source 对应环境，并验证桥接/接口边界",
        ]
    else:
        recommended_commands = [
            "未识别到明确工作区类型，请先检查 package.xml、CMakeLists.txt、setup.py 与现有构建脚本",
        ]
...
def print_human_readable(result: DetectionResult) -> None:
    ...
    for package in result.packages:
        joined_markers = ", ".join(package.markers) if package.markers else "none"
        print(f"  - {package.name}: {package.build_type} [{joined_markers}]")
